...
import { createIncomeBodyInputDto, createIncomeResponseDto } from '../../../../../Maui-Backend/src/controllers/types';
...
export const createNewIncome = async (data: createIncomeBodyInputDto) => {
  await setHeaders();
  const response = await MauiApi.post<createIncomeResponseDto>('/createNewIncome', data);

...
export const fetchContacts = async () => {
  ...
  const filteredContacts = contacts
    ...
    .sort((a, b) => {
      const n = a.name.toLocaleLowerCase().localeCompare(b.name.toLocaleLowerCase());
      return n === 0 && a.name !== b.name ? b.name.localeCompare(a.name) : n;
    });
  return filteredContacts;
};

export const checkPermissionAndroid = async () => {
  try {
...
export const checkCameraPermission = async () => {
  try {
    const checked = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.CAMERA);
    return checked;
  } catch (err) {
    console.warn(err);
  }
